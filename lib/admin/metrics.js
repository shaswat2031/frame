...
import { getMockDashboardSnapshot } from "@/lib/mock-feed";
...
export async function getDashboardSnapshot() {
  ...
  const [
    ...
    db.collection("orders")
      .find({}, { projection: { orderNumber: 1, customerName: 1, totalAmount: 1, status: 1, createdAt: 1 } })
      .sort({ createdAt: -1 })
      .limit(5)
      .toArray(),
  ]);

  const totals = [productStats.total, orderStats.total, userCount, ticketStats.total];
