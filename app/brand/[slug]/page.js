...
import { getMockCollectionBySlug, getMockProductsBySlug } from '@/lib/mock-feed';

export default function BrandPage() {
  const { slug } = useParams();
  const collection = useMemo(() => getMockCollectionBySlug(slug), [slug]);
  const products = useMemo(() => getMockProductsBySlug(slug), [slug]);
          ...
          <div className="container mx-auto">
            <header className="mb-16 flex flex-col md:flex-row md:items-end justify-between gap-8 border-b border-gold/10 pb-12">
              <div className="max-w-2xl">
                <span className="text-teal uppercase tracking-[0.5em] text-[10px] font-bold mb-4 block">Brand Archive</span>
              ...
              <div className="flex flex-col items-end text-right">
                <div className="text-[10px] uppercase tracking-widest text-gold mb-2">Products</div>
                ...
                >
                  <Link href={`/shop/${product.id}`} className="block">
                    ...
                    <div className="flex flex-wrap gap-2 mb-3">
                      <span className="border border-gold/20 px-2 py-1 text-[8px] font-mono tracking-[0.2em] uppercase text-gold">{collection.type === 'brand' ? product.brand : collection.name}</span>
                      <span className="border border-cream/10 px-2 py-1 text-[8px] font-mono tracking-[0.2em] uppercase text-cream/60">{collection.type === 'category' ? product.category : product.category}</span>
                    </div>
